...
class RNN(nn.Module):
    def __init__(self, type="LSTM", embedding_size=64, hidden_size=100, num_class=2, num_layers=1, attention_type=None, output_layer_type="linear", vocab=None, vectors=None, vocab_size=None):
        ...
        if self.output_layer_type == "linear":
            ...
            self.output = nn.Sequential( nn.Linear(hidden_size, hidden_size*2), nn.LeakyReLU(),
                                        nn.Dropout(0.5), nn.Linear(hidden_size*2, hidden_size), nn.LeakyReLU(),
    ...
    def forward(self, x: torch.Tensor, lengths: torch.LongTensor):
        ...
        if self.attention_type is not None:
            out_sequence, _ = torch.nn.utils.rnn.pad_packed_sequence(out_sequence,batch_first=True)
            if self.attention_type == "last_hidden_layer":
                ...
                # [n, 1, L] = [n, 1, H] x [n, H, L]
                att_weights = torch.bmm(hn.unsqueeze(1), out_sequence.transpose(1, 2))
                ...
                # self attetion
                # [n, L, L] = [n, L, H] x [n, H, L]
                att_weights = torch.bmm(out_sequence, out_sequence.transpose(1, 2))
                att_weights = torch.nn.functional.softmax(att_weights, dim=-1)
                # [n, L, H] = [n, L, L] x [n, L, H]
                ...
                # last hidden layer attention
                # [n, 1, L] = [n, 1, H] x [n, H, L]
                att_weights = torch.bmm(hn.unsqueeze(1), att_out_sequence.transpose(1, 2))
                att_weights = torch.nn.functional.softmax(att_weights, dim=-1)
                # [n, 1, H] = [n, 1, L] x [n, L, H]
