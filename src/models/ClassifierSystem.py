...
class LightningClassifier(LightningModule):
    def __init__(self, model_type="LSTM", embedding_size=64, hidden_size=100, num_class=2, batch_size=32, learning_rate=0.001, vocab=None, vectors=None, data_dir=".data"):
        super().__init__()
        self.model_type = model_type
        if model_type == "LSTM":
            self.model = LSTM(embedding_size=embedding_size, lstm_hidden_size=hidden_size, num_class=num_class, vocab=vocab, vectors=vectors)
        else:
